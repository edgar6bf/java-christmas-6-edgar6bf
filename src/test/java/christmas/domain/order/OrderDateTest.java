...
class OrderDateTest {
    ...
    @DisplayName("주문 날짜를 입력하면 OrderDate가 생성된다.")
    @Test
    void createOrderDate() throws Exception {
        // Given
        int orderDateValue = 25;

        // When
        OrderDate orderDate = new OrderDate(orderDateValue);

        // Then
        assertThat(orderDate).isNotNull();
    }

    @DisplayName("유효 범위에 속하지 않는 주문 날짜를 입력하면 예외가 발생한다.")
